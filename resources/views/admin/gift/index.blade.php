            ...
            <div class="row align-items-center">
                ...
                <div class="col-md-6">
                    <div class="d-flex flex-wrap align-items-center justify-content-end gap-2 mb-3">
                        <div>
                            <a href="{{ route('admin.gift.add') }}" class="btn btn-secondary"><i
                        ...
                        <tbody>
                            @foreach ($gifts as $row)
                                <tr>
                                    <td>{{ $row->name }}</td>
                                    <td>
                                        <dotlottie-wc src="{{ $row->link }}" style="width: 80px; height: 80px" autoplay
                                            loop>
                                        </dotlottie-wc>
                                    </td>
                                    <td>{{ $row->coins }}</td>
                                    ...
                                    <td>
                                        <a class="btn btn-primary btn-sm"
                                            href="{{ route('admin.gift.edit', ['id' => $row->id]) }}" id="delete">
                                            <i data-feather="edit" class="icon-lg"></i>
                                        </a>
                                        <!-- Delete Button -->
                                        <form action="{{ route('admin.gift.delete', ['id' => $row->id]) }}" method="POST"
